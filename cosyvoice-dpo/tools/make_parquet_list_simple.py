...
def job(utt_list, parquet_file):
    ...
    prompt_list = [utt2style_prompt[utt] for utt in utt_list]
...
if __name__ == "__main__":
    ...
    utt2wav, utt2text, utt2spk, utt2style_prompt = {}, {}, {}, {}
    ...
    with open('{}/style_prompts'.format(args.src_dir)) as f:
        for l in f:
            l = l.replace('\n', '').split()
            utt2style_prompt[l[0]] = ' '.join(l[1:])
    ...
    if args.dpo:
        utt2reject_speech_token = torch.load('{}/neg_utt2speech_token.pt'.format(args.src_dir))
    ...
    for i, j in enumerate(range(0, len(utts), args.num_utts_per_parquet)):
        ...
        pool.apply_async(job, (utts[j: j + args.num_utts_per_parquet], parquet_file))
    pool.close()
    pool.join()
