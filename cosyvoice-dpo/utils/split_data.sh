...
if [ ! -d $s1 ]; then
  ...
  # Re-split if utt2emotion exists and style_prompts in split is missing or older
  if [ -f $data/utt2emotion ] && [[ ! -f $s1/style_prompts || $s1/style_prompts -ot $data/utt2emotion ]]; then
    need_to_split=true
  fi
fi
...
# If utt2emotion exists, generate style_prompts (uttid style) and split it.
# Style: "Speaks in a neutral tone." for neutral; "Speaks in a strongly {emotion} tone." otherwise.
if [ -f $data/utt2emotion ]; then
  awk '{
    if ($2 == "neutral") {
      print $1 " Speaks in a neutral tone."
    } else {
      print $1 " Speaks in a strongly " $2 " tone."
    }
  }' $data/utt2emotion > $data/style_prompts
  utils/filter_scps.pl JOB=1:$numsplit \
    $data/split${numsplit}${utt}/JOB/utt2spk $data/style_prompts \
    $data/split${numsplit}${utt}/JOB/style_prompts || exit 1
fi
