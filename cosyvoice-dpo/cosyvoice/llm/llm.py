...
class Qwen2LM(TransformerLM):
    ...
    def prepare_lm_input_target(self, text_token, text_token_emb, text_token_len, speech_token, speech_token_emb, speech_token_len):
        ...
        for i in range(len(text_token)):
            # bistream sequence (skip if text_token_len is 0 to avoid division by zero)
            if (text_token_len[i] > 0 and random.random() < 0.5 and
                    speech_token_len[i] / text_token_len[i] > self.mix_ratio[1] / self.mix_ratio[0]):
    ...
    def forward(
        ...
        # Validate finite inputs to catch numerical issues before they cause SIGFPE
        if not torch.isfinite(text_token_emb).all() or not torch.isfinite(speech_token_emb).all():
            raise RuntimeError("Non-finite values in embeddings (NaN/Inf), skipping batch to avoid floating point exception")
    ...
    def forward_dpo(
        ...
        # Validate finite inputs to catch numerical issues before they cause SIGFPE
        if not torch.isfinite(text_token_emb).all() or not torch.isfinite(speech_token_combined_emb).all():
            raise RuntimeError("Non-finite values in embeddings (NaN/Inf), skipping batch to avoid floating point exception")
        ...
        chosen_mask_sum = chosen_lm_mask.sum(dim=-1).clamp(min=1e-8)
        rejected_mask_sum = rejected_lm_mask.sum(dim=-1).clamp(min=1e-8)
        chosen_logps = (chosen_logps * chosen_lm_mask).sum(dim=-1) / chosen_mask_sum
        rejected_logps = (rejected_logps * rejected_lm_mask).sum(dim=-1) / rejected_mask_sum
