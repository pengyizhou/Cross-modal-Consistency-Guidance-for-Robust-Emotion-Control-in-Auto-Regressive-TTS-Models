    ...
    with open(json_path, 'r') as f:
        ...
        for line in json_lines:
            ...
            for trial in range(max_trials):
                try:
                    set_all_random_seed(42 + trial * 1000)
                    ...
                    if trial == max_trials - 1:
                        print(f"[WARN] Skipping sample {json_data['ID']} after {max_trials} failed trials: {e}")
                    else:
                        print(f"[WARN] RuntimeError for {json_data['ID']}, trial {trial + 1}/{max_trials}, retrying with different seed: {e}")
