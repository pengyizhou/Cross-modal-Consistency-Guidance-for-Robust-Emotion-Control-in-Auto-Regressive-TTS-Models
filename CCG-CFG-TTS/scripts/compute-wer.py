...
def normalize(sentence, ignore_words, cs, split=None):
    ...
    """
    new_sentence = []
    # first normalize 126 into "one hundred and twenty six"
    # sentence = sentence.replace("126", "one hundred twenty six")
...
if __name__ == '__main__':
    ...
    with codecs.open(hyp_file, 'r', 'utf-8') as fh:
        for line in fh:
            ...
            fid = array[0]
            content = " ".join(array[1:])
            content = content.replace("126", "one hundred twenty six")
            content = content.replace("pebblecovered", "pebble covered")
            content = content.replace("letterbox", "letter box")
            content = content.replace("passbook", "pass book")
            content = content.replace("chequebook", "cheque book")
            content = content.replace("everyone", "every one")
            content = content.replace("tomorrow", "to morrow")
            content = content.replace("briefcase", "brief case")
            content = content.replace("panicstricken", "panic stricken")
            content = content.replace("lifeblood", "life blood")
            content = content.replace("fullgrown", "full grown")
            content = content.replace("brainpan", "brain pan")
            array = content.split()
            rec_set[fid] = normalize(array, ignore_words, case_sensitive,
                                     split)
